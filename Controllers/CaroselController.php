...
class CaroselController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user_access_object = new CaroselAccessor();
    }
    ...
    {
        try {
            $data = $this->user_access_object->get_all_carsol();
            $all_carosels = array();
            foreach ($data as $item) {
                $parsed_user = $this->parse_object($item);
                $all_carosels[] = $parsed_user;
            }
            $this->response_data = json_encode($all_carosels);
    ...
    private function get_by_id($id)
    {
        try {
            ...
            $data = $this->user_access_object->get_carsol($id);
            $all_carosels = array();
            foreach ($data as $item) {
                $parsed_carosel = $this->parse_object($item);
                $all_carosels[] = $parsed_carosel;
            }
            $this->response_data = json_encode($all_carosels);
            $this->responseHeaders = ['Content-Type: application/json', 'HTTP/1.1 200 OK'];
        } catch (PDOException $ex) {
    ...
    private function push()
    {
        try {
            ...
            if
            (isset($u_request["title"]) && isset($u_request["description"]) && isset($u_request["image_link"]))  {
                $Carosel = new Carosel();
                $Carosel->set_title($u_request["title"]);
                $Carosel->set_description($u_request["description"]);
                $Carosel->set_image_link($u_request["image_link"]);
                $this->user_access_object->create_carsol($Carosel);
    ...
    private function put()
    {
        try {
            ...
            if
            (isset($u_request["title"]) && isset($u_request["description"]) && isset($u_request["image_link"]) && isset($u_request["id"]))  {
                $Carosel = new Carosel();
                $Carosel->set_title($u_request["title"]);
                $Carosel->set_description($u_request["description"]);
                $Carosel->set_image_link($u_request["image_link"]);
                $Carosel->set_id($u_request["id"]);
                $this->user_access_object->update_carsol($Carosel);
    ...
    private function delete()
    {
        try {
            ...
            if (isset($u_request["id"])) {
                $this->user_access_object->delete_carsol($u_request["id"]);
