...
class SkillController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user_access_object = new SkillAccessor();
    }
    ...
    {
        try {
            $data = $this->user_access_object->get_all_skill();
            $all_skills = array();
            foreach ($data as $item) {
                $parsed_user = $this->parse_object($item);
                $all_skills[] = $parsed_user;
            }
            $this->response_data = json_encode($all_skills);
    ...
    private function get_by_id($id)
    {
        try {
            ...
            $data = $this->user_access_object->get_skill($id);
            $all_skills = array();
            foreach ($data as $item) {
                $parsed_skill = $this->parse_object($item);
                $all_skills[] = $parsed_skill;
            }
            $this->response_data = json_encode($all_skills);
            $this->responseHeaders = ['Content-Type: application/json', 'HTTP/1.1 200 OK'];
        } catch (PDOException $ex) {
    ...
    private function push()
    {
        try {
            ...
            if (isset($u_request["name"]) && isset($u_request["iconname"]) && isset($u_request["expertiselevel"]) ) {
                $skill = new Skill();
                $skill->set_name($u_request["name"]);
                $skill->set_expertise($u_request["expertiselevel"]);
                $skill->set_icon($u_request["iconname"]);
                $this->user_access_object->create_skill($skill);
    ...
    private function put()
    {
        try {
            ...
            if(isset($u_request["name"]) && isset($u_request["iconname"]) && isset($u_request["expertiselevel"])&& isset($u_request["id"]) ) {
                $skill = new Skill();
                $skill->set_id($u_request["id"]);
                $skill->set_name($u_request["name"]);
                $skill->set_expertise($u_request["expertiselevel"]);
                $skill->set_icon($u_request["iconname"]);
                $this->user_access_object->update_skill($skill);
    ...
    private function delete()
    {
        try {
            ...
            if (isset($u_request["id"])) {
                $this->user_access_object->delete_skill($u_request["id"]);
    ...
    private function parse_object($data)
    {
        return [
            "id" => $data->get_id(),
            "name" => $data->get_name(),
            "expertiseLevel" => $data->get_expertise(),
            "iconname" => $data->get_icon()
        ];
    }
