...
class ServiceController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->user_access_object = new ServiceAccessor();
    }
    ...
    private function get()
    {
        try {
            ...
            $data = $this->user_access_object->get_all_service();
            $all_services = array();
            foreach ($data as $item) {
                $parsed_user = $this->parse_object($item);
                $all_services[] = $parsed_user;
            }
            $this->response_data = json_encode($all_services);
    ...
    private function get_by_id($id)
    {
        try {
            ...
            $data = $this->user_access_object->get_service($id);
            $all_services = array();
            foreach ($data as $item) {
                $parsed_service = $this->parse_object($item);
                $all_services[] = $parsed_service;
            }
            $this->response_data = json_encode($all_services);
            $this->responseHeaders = ['Content-Type: application/json', 'HTTP/1.1 200 OK'];
        } catch (PDOException $ex) {
    ...
    private function push()
    {
        try {
            ...
            if (isset($u_request["name"]) && isset($u_request["iconname"]) && isset($u_request["description"])) {
                $service = new Service();
                $service->set_name($u_request["name"]);
                $service->set_description($u_request["description"]);
                $service->set_icon($u_request["iconname"]);
                $this->user_access_object->create_service($service);
    ...
    private function put()
    {
        try {
            ...
            if (isset($u_request["name"]) && isset($u_request["iconname"]) && isset($u_request["description"])&& isset($u_request["id"])) {
                $service = new Service();
                $service->set_id($u_request["id"]);
                $service->set_name($u_request["name"]);
                $service->set_description($u_request["description"]);
                $service->set_icon($u_request["iconname"]);
                $this->user_access_object->update_service($service);
    ...
    private function delete()
    {
        try {
            ...
            if (isset($u_request["id"])) {
                $this->user_access_object->delete_service($u_request["id"]);
